...
这个流程的目标不是填表，而是让用户始终知道：今天推进的是现实水产系统的哪一层、映射成了哪种 C 结构、用什么运行结果证明它真的成立。
...
- 先给函数签名、伪代码和验收标准。
- 核心代码必须由用户亲手写。
- 用户贴代码后，Codex 再审查、校准、验证。
- 不允许过度设计。
- 不允许在当前阶段引入数据库、前端、云平台、AI、机器学习、硬件驱动。
- 不允许大规模重构。
...
- 不默认展开固定的编译命令和测试方法。
- 需要验证时，Codex 直接运行编译/测试，并简述结果。
- 只有用户明确要求、收口、提交建议或验证失败需要复现时，才展开具体命令。
