# 004 - CSV Store Module v0

## 0. Metadata

packet_id: 004-csv-store-module-v0
stage: Stage 2-1
task_name: CSV Store Module v0
...
```text
用户本人手动拆出 csv_store 模块 v0，只负责把 PondRecord 追加写入 CSV 文件。
```

今日主线不是做完整日志系统，不是写数据库，不是写时间戳，不是写表头只写一次，不是接 main，不是接硬件。

今日完整闭环是：

```text
上一个闭环 SRS 抽卡
→ 用户手动阅读 record / judge / control / Makefile
→ Codex 解释 csv_store 模块边界和建议接口
→ 用户手动创建 include/csv_store.h
→ 用户手动创建 src/csv_store.c
→ 用户手动创建 tests/test_csv_store.c
→ 用户手动修改 Makefile 增加 test_csv_store
→ 用户手动运行 make test_csv_store
→ 用户手动运行 make test
...
```text
csv_store 暂时不接 judge/control。
csv_store 只保存 PondRecord 原始字段。
```
...
```text
将一条 PondRecord 追加写入 filename 指向的 CSV 文件。
成功返回 0。
失败返回非 0。
```
...
```text
1. FILE* 文件操作
2. fopen / fprintf / fclose
3. 追加写入 append
4. CSV 文本序列化
5. 文件路径与 build/ 临时测试文件
6. 返回值错误处理
7. Makefile 多测试目标
8. Bug Lab 文件模式错误注入
9. SRS 抽卡
...
31. 不修改 include/record.h。
32. 不修改 include/judge.h。
33. 不修改 include/control.h。
34. 不修改 src/judge.c。
35. 不修改 src/control.c。
36. 不整理无关缩进。
37. 不格式化无关代码。
38. 不补充无关注释。
39. 不做顺手优化。
...
```text
最小 csv_store 追加写入模块 + 闭环末端 Bug Lab 训练
```
...
3. csv_store 不得新增任何温度状态解释。
4. csv_store 不得新增任何 DO 阈值解释。
5. csv_store 不得新增控制规则。
6. 不得新增温度阈值。
7. 不得关闭 GAP-001。
...
```text
1. docs/daily/2026-05-25-control-module-v0.md
2. bug_lab/2026-05-25-control-interface-contract.md
3. control 接口合同 bug
4. control 模块边界
5. 上一次 408 映射薄弱点
```
...
```text
Q1: control 模块为什么调用 needs_aeration(record)，而不是自己写 oxygen < 5.0f？
A1: 用户回答后 Codex 校准。

Q2: control.h 和 control.c 的函数签名不一致会导致什么错误？
A2: 用户回答后 Codex 校准。

Q3: test_control 为什么需要同时编译 src/control.c 和 src/judge.c？
A3: 用户回答后 Codex 校准。

Q4: judge / control / hardware 的职责边界是什么？
A4: 用户回答后 Codex 校准。

Q5: csv_store 为什么不应该调用 judge 或 control？
A5: 用户回答后 Codex 校准。
```
...
5. 阅读 include/control.h。
6. 阅读 src/judge.c。
7. 阅读 src/control.c。
8. 阅读 tests/test_judge.c。
9. 阅读 tests/test_control.c。
10. 手动创建 include/csv_store.h。
11. 手动创建 src/csv_store.c。
12. 手动创建 tests/test_csv_store.c。
13. 手动修改 Makefile。
14. 手动运行 make test_csv_store。
15. 手动运行 make test。
16. 手动运行 git diff --check。
17. 手动运行 Clean Baseline 检查。
18. 手动注入 Bug Lab bug。
19. 手动触发 bug。
20. 手动修复 bug。
21. 手动写 bug_lab。
22. 手动写 docs/daily。
23. 手动 git add / commit。
```
...
# 2. 阅读现有 record / judge / control / Makefile
sed -n '1,160p' include/record.h
sed -n '1,160p' include/judge.h
sed -n '1,160p' include/control.h
sed -n '1,200p' src/judge.c
sed -n '1,200p' src/control.c
sed -n '1,200p' tests/test_judge.c
sed -n '1,200p' tests/test_control.c
sed -n '1,220p' Makefile

# 3. 手动创建 csv_store 接口
nvim include/csv_store.h

# 4. 手动创建 csv_store 实现
nvim src/csv_store.c

# 5. 手动创建 csv_store 测试
nvim tests/test_csv_store.c

# 6. 手动修改 Makefile，增加 test_csv_store
nvim Makefile
...
# 9. Clean Baseline 检查
git diff -- tests/test_judge.c tests/test_control.c
git diff -- include/record.h include/judge.h include/control.h
git diff -- src/judge.c src/control.c
...
int csv_store_append_record(const char *filename, PondRecord record)
{
    ...
    fprintf(fp, "%d,%.2f,%.2f\n", record.pond_id, record.temp, record.oxygen);
...
注意：字段必须以当前 `include/record.h` 为准。如果 `PondRecord` 字段名不是 `pond_id/temp/oxygen`，用户必须按真实字段修改，不得编造字段。

建议 test_csv_store.c 最小测试方向：

```text
1. 使用 build/test_pond_log.csv 作为临时测试文件。
2. 写入两条 PondRecord。
3. 确认 csv_store_append_record 返回 0。
4. 重新打开文件读取内容。
5. 确认文件至少包含两行记录。
6. 不测试表头。
7. 不测试时间戳。
...
test_csv_store:
	@mkdir -p build
	$(CC) $(CFLAGS) -Wextra -Iinclude src/csv_store.c tests/test_csv_store.c -o build/test_csv_store
...
```makefile
test: test_judge test_control test_csv_store
```
...
10. 直接注入 bug。
11. 直接修复 bug。
12. 直接创建 include/csv_store.h。
13. 直接创建 src/csv_store.c。
14. 直接创建 tests/test_csv_store.c。
```
...
```text
1. 用户本人完成 include/csv_store.h。
2. 用户本人完成 src/csv_store.c。
3. 用户本人完成 tests/test_csv_store.c。
4. 用户本人完成 Makefile 修改。
5. 用户本人运行 make test_csv_store。
6. 用户本人运行 make test。
7. 测试通过。
8. git diff 只包含目标文件。
9. tests/test_judge.c 无 diff。
10. tests/test_control.c 无 diff。
11. include/record.h 无 diff。
12. include/judge.h 无 diff。
13. include/control.h 无 diff。
14. src/judge.c 无 diff。
15. src/control.c 无 diff。
16. domain_snapshot 无 diff。
17. exercises/06-projects/1.c 无 diff。
18. csv_store 不调用 judge。
19. csv_store 不调用 control。
20. csv_store 不写领域阈值。
21. csv_store 不处理 temp_status。
22. csv_store 不做表头只写一次。
23. csv_store 不做时间戳。
24. csv_store 不接 main。
25. 没有无关格式化。
26. 没有 Codex 代工。
27. docs/daily 有真实操作记录。
28. SRS 有新增候选。
29. Bug Lab 完成受控注入和修复。
30. 408 映射完成，并尽量细到王道小节。
```
...
make test: 通过
git diff --check: 通过
tests/test_judge.c/tests_test_control.c: no diff
include/record.h/include/judge.h/include/control.h: no diff
src/judge.c/src/control.c: no diff
...
7. 无 record / judge / control 既有文件误改。
8. 无 build/、tmux log 等杂项进入暂存区。
```
...
6. 不破坏 Git 历史。
7. 不扩大主线范围。
8. 能训练 C / 文件系统 / Makefile / Git / 408 中至少一个知识点。
9. 用户亲自注入。
10. 用户亲自修复。
```
...
```text
A. fopen 模式从 "a" 临时改成 "w"
B. fclose(fp) 临时删除
C. Makefile 少链接 src/csv_store.c
```
...
bug_lab_recommendation:
  type: file_mode_error
  name: csv_append_vs_overwrite
  injection_point: src/csv_store.c 的 fopen 模式
  injection_action: 用户手动临时把 fopen(filename, "a") 改成 fopen(filename, "w")
  trigger_command: make test_csv_store
  expected_error: 第二次写入覆盖第一次写入，导致测试读取到的记录行数不足或内容不符合预期
  repair_action: 用户手动恢复为 fopen(filename, "a")
  risk_level: L3
  domain_risk: none
```
...
1. 理解 "a" 是追加写入。
2. 理解 "w" 会截断并覆盖原文件。
3. 理解养殖记录系统为什么需要追加而不是覆盖。
4. 理解 CSV 是项目的最小持久化层。
5. 理解文件操作 bug 如何通过测试暴露。
```
...
```text
1. 用户手动注入 bug。
2. 用户运行 make test_csv_store。
3. 用户复制报错或异常输出。
4. 用户先自己判断错误类型。
5. Codex 只做提示，不直接给最终答案。
6. 用户手动修复。
7. 用户重新运行 make test_csv_store。
8. 用户重新运行 make test。
9. 用户写 bug_lab。
10. Codex 审查 bug_lab。
```
...
compile / link / runtime / make / file_io / git / domain_policy

本次预期：file_io / behavior regression

## 8. 根因分析

解释为什么 "w" 会覆盖文件，而 "a" 会追加到文件末尾。
...
本次 csv_store 模块创建、Makefile 修改、测试命令执行、Bug Lab 注入、Bug 修复、docs/daily 与 bug_lab 初稿均由用户本人完成。Codex 仅提供带练、审查和纠偏。

## 1. 今日主线

用户本人手动拆出最小 csv_store 模块 v0。
...
9. 是否明确 record / judge / control / csv_store / main 的职责边界。
```
...
A1: "a" 表示追加写入，写入位置在文件末尾；"w" 表示写入并截断原文件，会覆盖历史内容。

Q2: csv_store 为什么不应该调用 judge/control？
A2: 因为 csv_store 是持久化层，只保存记录；judge 负责判断，control 负责建议，混在一起会破坏模块边界。

Q3: FILE* 返回 NULL 通常说明什么？
A3: 文件打开失败，可能由路径不存在、权限不足、文件名无效等原因导致。

Q4: 为什么 CSV 写入属于操作系统文件管理映射？
A4: 因为 fopen/fprintf/fclose 最终依赖文件系统完成文件打开、写入和关闭。

Q5: 为什么养殖记录系统需要 append，而不是 overwrite？
A5: 因为养殖数据需要长期历史记录，覆盖会丢失之前的观测数据，破坏后续复盘和趋势分析。
```
...
408_mapping:
  data_structure:
    ...
    - chapter: "第1章 绪论"
      section: "抽象数据类型"
      point: "数据对象与基本操作"
      project_mapping: "csv_store_append_record(filename, record) 是对 PondRecord 执行持久化操作。"
      task_evidence: "include/csv_store.h"

    - chapter: "第2章 线性表"
      section: "线性表的定义和基本操作"
      point: "顺序记录与追加"
      project_mapping: "CSV 文件中的多行记录可视为外存上的顺序记录集合。"
      task_evidence: "tests/test_csv_store.c 写入多条记录"
  ...
  computer_organization:
    - chapter: "第1章 计算机系统概述"
      ...
      task_evidence: "Makefile 中 test_csv_store 目标"

    - chapter: "第2章 数据的表示和运算"
      section: "整数与浮点数表示"
      point: "int / float 数据的文本化输出"
      project_mapping: "fprintf 将 PondRecord 中的 pond_id、temp、oxygen 输出为 CSV 文本。"
      task_evidence: "src/csv_store.c 中 fprintf"

    - chapter: "needs_manual_alignment_with_wangdao"
      section: "内存数据与外存表示"
      point: "结构体字段从内存表示转换为文本文件表示"
      project_mapping: "PondRecord 在内存中是结构体，写入 CSV 后变成外存文本记录。"
      task_evidence: "csv_store_append_record"
  ...
  operating_system:
    ...
    - chapter: "第4章 文件管理"
      section: "文件打开方式"
      point: "追加写入与覆盖写入"
      project_mapping: "\"a\" 追加记录，\"w\" 覆盖原文件；Bug Lab 通过该差异训练文件模式理解。"
...
```bash
git commit -m "feat: add csv store module v0"
git commit -m "docs: record csv store file mode bug lab"
```
...
```markdown
# Codex 第一轮：004-csv-store-module-v0 执行计划
...
判断 record / judge / control 是否稳定。
判断 Makefile 是否已有 test_csv_store。
判断 csv_store 模块是否已存在。
判断领域边界。

## 4. 今日主线最小目标

说明今日只做最小 csv_store 追加写入模块。
...
明确不允许 Codex 创建 csv_store 文件、修改 Makefile、写 docs、写 bug_lab、修改 C 文件、修改 domain_snapshot。
...
7. SRS 是否来自真实问题。
8. 408 映射是否细到王道小节。
9. csv_store 是否只保存 PondRecord 原始字段，没有调用 judge/control。
10. 下一步用户亲自做什么。
```
...
5. 替用户注入 bug。
6. 替用户修 bug。
7. 替用户创建 csv_store 模块。
```

最终完成标准：

```text
用户本人完成 csv_store 模块创建。
用户本人完成 Makefile 修改。
用户本人完成主线测试。
